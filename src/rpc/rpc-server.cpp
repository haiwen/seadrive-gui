...
QString getAppletRpcPipePath()
{
    ...
    // return toCStr(QDir(gui->daemonManager()->currentCacheDir()).filePath(kSeaDriveSockName));  
#endif
}
...
public:
    bool connect() {
        seafile_rpc_client_ = createSearpcClientWithPipeTransport(kSeaDriveRpcService);
